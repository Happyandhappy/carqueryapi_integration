//Definition of CarQuery     
	...
	CarQuery.prototype = {
		...
		drive: 		"",
		drive_id: 		"cq-drive",    
		empty_option_html: 	"<option value=''>Please select	</option>",    
		...
		$("input#"+year_select_id).bind('change', function(){sender.yearSelectChange();});
		...
		initYearMakeModelTrim: function(year_select_id, make_select_id, model_select_id, trim_select_id, year_data_id, makes_data_id, models_data_id,trim_data_id)
		{
			//Set the ids for the select elements
			this.year_select_id =  year_select_id;
			this.make_select_id =  make_select_id;
			...
			this.year_data_id = year_data_id;
			this.makes_data_id = makes_data_id;
			this.models_data_id = models_data_id;
			this.trim_data_id = trim_data_id;
			//Populate the car-years select element
			this.populateYearSelect();
			...
			//Set the change event for the makes dropdown to populate the models select
			$("input#"+make_select_id).bind('change', function(){sender.makeSelectChange();});
			
			//Set the change event for the models dropdown to populate the trims select
			$("input#"+model_select_id).bind('change', function(){sender.modelSelectChange();});
			
			//Set the change event for the trim dropdown to save the selected trim
			$("input#"+trim_select_id).bind('change', function(){sender.trimSelectChange();});
		},
		...
		{    
			//Set a loading message while we retrieve the data
			// $("datalist#"+this.year_data_id).html("<option value=''>Loading Years...</option>");
			...
			if(!sender.responseError(data))
			{
				...
				$("datalist#"+sender.year_select_id).html(options);
				
				$("datalist#"+sender.makes_data_id).html(sender.empty_option_html);
				$("datalist#"+sender.models_data_id).html(sender.empty_option_html);
			
			
			if(sender.settings.year != null)
			{
					$('input#'+sender.year_select_id).val(sender.settings.year);
		...
		if(!this.responseError(data))
		{
			...
			$("datalist#"+this.makes_data_id).html(options);
		}
		},
		...
		if(this.settings.model != null && this.settings.model == models[i].model_name) s = 'selected="selected"';
			...
			$("datalist#"+this.models_data_id).html(options);
		},
		...
		{    
			...
			$("datalist#"+this.trim_data_id).html(options);
			
			this.cur_trim = $("input#"+this.trim_select_id).val();
		...
		{        
			this.cur_year = $("input#"+this.year_select_id).val();
			...
			if(this.cur_year == "")
			{
				$("input#"+this.make_select_id).val("");
				$("input#"+this.model_select_id).val("");
				$("input#"+this.trim_select_id).val("");
					return;
			}
		...
		{
			this.cur_make = $("input#"+this.make_select_id).val();
		...
		$.getJSON(this.base_url+"?callback=?", {cmd:"getModels", make:this.cur_make, year:this.cur_year, sold_in_us:this.sold_in_us}, function(data) {
			...
			if(!sender.responseError(data))
			{
					...
					sender.cur_model = $('input#'+sender.model_select_id).val();
		...
		{
			this.cur_model = $("input#"+this.model_select_id).val();
			...
			$.getJSON(this.base_url+"?callback=?", {cmd:"getTrims", make:this.cur_make, year:this.cur_year, model:this.cur_model, sold_in_us:this.sold_in_us, full_results:0 }, function(data) {
				...
				if(!sender.responseError(data))
					...
					sender.cur_trim = $('input#'+sender.trim_select_id).val();
			});
		},
...
// Holds Edmunds API response data
	var edmunds = [null, null, null, null, null];
	var vehicles = 1;
	var drivers = 1;
	...
	function cloneVehicle() {
		...
		// Clone Vehicle HTML fieldset
		var fieldsets = document.getElementsByClassName("VEHICLES");
		var fieldset = fieldsets[fieldsets.length - 1];
		var newFieldset = fieldset.cloneNode(true);
		var updateMe = newFieldset.getElementsByTagName("*")
		var update_yearlist;
		var update_makelist;
		var update_modellist;
		var update_trimlist;

		// Clear data from Clone
		for (var c = 0; c < updateMe.length; c++) {
			var element = updateMe[c];
			if (element.nodeName == "INPUT") {
				if (element.value != "" && element.type != "checkbox" && element.type
!= "button") {
					element.value = "";
				}
				...
				if (element.name.indexOf("Vehicle" + (vehicles - 1)) >= 0) {
					element.name = element.name.replace(vehicles - 1, vehicles);

				}
				if (element.className.indexOf("Vehicle" + (vehicles - 1)) >= 0) {
					element.className = element.className.replace(vehicles - 1, vehicles);
					element.id =  element.id.replace(vehicles - 1, vehicles);
					element.setAttribute('list',element.list.id.replace(vehicles-1,vehicles));
				}
		...
		// Insert clone
		var form1 = document.getElementById("form-container");
		form1.insertBefore(newFieldset, fieldset.nextSibling);
		initial(vehicles);
	}
	...
	function cloneDriver() {
		...
		// Clone Driver HTML fieldset
		var fieldsets = document.getElementsByClassName("DRIVERS");
		var fieldset = fieldsets[fieldsets.length - 1];
		var newFieldset = fieldset.cloneNode(true);
		var updateMe = newFieldset.getElementsByTagName("*")
...
!= "button") {
				...
				if (element.name.indexOf("Driver" + (drivers - 1)) >= 0) {
					element.name = element.name.replace(drivers - 1, drivers);
				}
				if (element.className.indexOf("Driver" + (drivers - 1)) >= 0) {
					element.className = element.className.replace("Driver" + (drivers -
1),"Driver" + drivers);
				}
			}
		}
...
function initial(value){
		...
		console.log('car-years'+value);
		carquery.init();
		carquery.initYearMakeModelTrim('car-years'+value, 'car-makes'+value, 'car-model'+value, 'car-model-trims'+value,'Vehicle'+value+'Year', 'Vehicle'+value+'Make','Vehicle'+value+'Model', 'Vehicle'+value+'Trim');
...
$(document).ready(function(){
    initial(1);
      ...
      $("#form1").submit(function() {
        $("input:checkbox:not(:checked)").val('No');
